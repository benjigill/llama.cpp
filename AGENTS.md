...
- local ngram-mod table size + wipe logging - `--spec-ngram-mod-size` (MiB, default 16 = upstream) so a long-running server keeps more shared history; (the occupancy wipe is gone, see the fingerprinted table below); a low-acceptance streak pauses ngram drafts for that slot (16..1024 rounds, MTP drafts meanwhile) instead of wiping the shared table, which the next draft rebuilt from the same context anyway
- local on-device spec checkpoints under -sm tensor - env `LLAMA_SPEC_CKPT_ON_DEVICE=1`; a draft longer than n_rs_seq (e.g. ngram-mod) copies the whole recurrent state to host and back each round. Upstream turned on-device checkpoints off in #24108 as not meta-safe; fixed here with a per-device meta `cpy_tensor`, whole-row views, copy tensors named after their source and no reuse of stale views. Costs one extra recurrent state per slot of VRAM that is not counted at startup
...
- #22587 row-per-warp GDN recurrent kernel - ported onto our file (PDL, cache fusion and rollback snapshots kept); kernel 1.6x at 64 tokens, flat at 1 token, end to end flat on the bench suites (helps 2-130 token batches below the chunked threshold)
